...
md_files = list.files(path = 'rweekly.org-gh-pages/_posts',
                      pattern="*.md")
md_files_full = list.files(path = 'rweekly.org-gh-pages/_posts',
                           pattern="*.md",
                           full.names = TRUE)

ifelse(!dir.exists(file.path('md-to-csv-files')), 
       dir.create(file.path('md-to-csv-files')), FALSE)
...
for (f in 1:length(md_files_full))  {
  ...
  mdown <- mdown %>% 
    mutate(section_lookup = grepl('##',value),
      section_title = ifelse(section_lookup == TRUE, value, NA )) %>%
    fill(section_title, .direction = 'down') %>%
    filter(section_lookup == FALSE) 
  
   mdown <-  mdown %>%
      mutate(type = ifelse(substring(mdown$value, 1, 3) == '+ [' & 
                           grepl('\\]', mdown$value) &
                           grepl('\\(', mdown$value) &
                           grepl('\\)', mdown$value),
                           'link',
                           ifelse(substring(mdown$value,1,2) == '![',
                                'image',
                                'unid')))
   
  issue_long = md_files[f]
  issue_regexp = regexpr("[0-9]+-[0-9]+-[0-9]+-",issue_long)
  mdown <- mutate(mdown, issue = substring(issue_long, 1, attributes(issue_regexp)$match.length-1))
